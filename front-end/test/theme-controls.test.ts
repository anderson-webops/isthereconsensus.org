...
describe("theme controls", () => {
	for (const file of layoutFiles) {
		it(`keeps the compact appearance controls in the footer in ${file}`, () => {
			...
			assert.match(
				footer,
				/<ClientOnly>[\s\S]*class="site-footer__appearance" aria-label="Appearance controls"[\s\S]*site-footer__appearance-label[\s\S]*Appearance[\s\S]*site-footer__appearance-controls[\s\S]*<PaletteSwitcher \/>\s*<ThemeToggle \/>/
			);
		});
