...
function emptySelection(): LibrarySelection {
	return { savedReviewIds: [], followedTopicIds: [] };
}

export function validLibrarySelection(value: unknown): value is LibrarySelection {
	if (!value || typeof value !== "object") return false;
	const record = value as Record<string, unknown>;
	return (["savedReviewIds", "followedTopicIds"] as const).every((key) => {
		const values = record[key];
		return (
			Array.isArray(values) &&
			values.length <= (key === "savedReviewIds" ? MAX_SAVED_REVIEWS : MAX_FOLLOWED_TOPICS) &&
			new Set(values).size === values.length &&
			values.every((id) => typeof id === "string" && /^[a-f\d]{24}$/.test(id))
		);
	});
}
...
export function decodeDeviceLibrary(raw: string | null): LibrarySelection {
	...
	if (!validLibrarySelection(value) || (value as unknown as { version: unknown }).version !== 1) {
		throw new Error("Invalid browser library");
	}
	return { savedReviewIds: [...value.savedReviewIds], followedTopicIds: [...value.followedTopicIds] };
}
...
export function createReaderLibrary(ports: LibraryPorts) {
	...
	async function mutate(transform: (current: LibrarySelection) => LibrarySelection, clear = false) {
		...
		try {
			if (state.scope === "device") {
				...
				ports.storage().setItem(READER_LIBRARY_KEY, JSON.stringify({ version: 1, ...next }));
				assign(next);
			} else {
			...
			state.error =
				statusCode(error) === 422
					? "A selected review or topic is no longer available. Reload your library before retrying."
					: state.scope === "account"
	...
	async function clearLibrary() {
		if (state.scope === "device" && !state.busy) {
			try {
				ports.storage().setItem(READER_LIBRARY_KEY, JSON.stringify({ version: 1, ...emptySelection() }));
				await reload();
