...
This checklist complements the automated `npm run a11y` axe smoke suite. Run it before shipping consensus browsing, claims, explainers, methodology, source standards, privacy, and terms changes.

## Screen Reader Pass

- macOS VoiceOver: open `/`, `/consensus`, `/standards`, `/source-standards`; use `VO + Right Arrow` through each page, then jump by headings, landmarks, buttons, form controls, and links.
...
- Verify visible focus on header navigation, topic links, claim cards, search or ask controls, disclosure sections, forms, and footer links.
